layui.define(['element', 'form'], function(exports) {
        ...
        $.ajax({
            ...
            success: function(res) {
                layer.msg(res.message, {},function() {
                    if(res.action=='login'){
                        $src=$(".login-code").attr('src');
                        $(".login-code").attr('src',$src+'?'+Math.random());
                    }
                    if ((res.url).length > 0) {
    ...
    //单条升级按钮
    $('.j-tr-up-lv').click(function(){

        var that = $(this),
            href = !that.attr('data-href') ? that.attr('href') : that.attr('data-href');
        layer.confirm('确认为该用户升级？', {title:false, closeBtn:0}, function(index){
            if (!href) {
                layer.msg('请设置data-href参数');
                return false;
            }
    ...
    $('.j-up-lv-menu').click(function(){
        ...
        layer.confirm('确定为所选用户升级？', {title:false, closeBtn:0}, function(index){
            layer.msg('数据提交中...',{time:500000});
            $.ajax({
                type: "POST",
                url: url,
