...
//    private $appId = 'a856dde9d4a343b186b12de4278d78fa';
    ...
    //回调地址
    private $notify_url = 'http://sbts.ifc007.com/index.php/admin/callback';
    //跳转地址
    private $return_url = 'http://sbts.ifc007.com/index.php/api/callback/pxback';
...
//        $rs['ssss'] = $postarr;
    ...
    function hxpay($user_data,$card_data,$debit_card,$order_data,$rate_data){
        ...
        //单笔消费交易手续费 必填
        $postarr['counter_fee_t0'] = (string)$order_data['extra_rate'];
        //费率 必填
        $postarr['rate_t0'] = (string)($user_data['settle_rate']*100);
        //备注
        $postarr['memo'] = '';
    ...
    public function bank_4(){
        ...
        //订单金额 必填
        $postarr['trans_amount'] = '10000';
        //姓名 必填
        $postarr['family_name'] = '张良钦';
        //身份证号 必填
        $postarr['id_card'] = '352228199604260517';
        //卡号 必填
        $postarr['bank_no'] = '6225768769374974';
        ...
        $url = "http://api.mypays.cn/api/service.json";
        //echo $url;
        $headers[] = 'Content-Type:application/json';
        $rs = $this->http($url,json_encode($data),'POST',$headers);
        //dump($rs);
        ...
        $filename=$path = $path."hx_log.txt";
        $fh = fopen($filename, "a+");
        $word = "\n".date('Y--m-d H:i:s',time()).json_encode($rs);
        fwrite($fh, $word);    // 输出：6
        fclose($fh);
        if(is_object($rs)){
            $rs=$this->object_to_array($rs);
